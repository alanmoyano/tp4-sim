...
export default function PoissonPage() {
  ...
  const generateNumbers = async () => {
    ...
    try {
      const response = await fetch(`/api/random/poisson?lambda=${lambda}&count=${count}`);
  ...
  const formatData = (data: number | number[]) => {
    if (Array.isArray(data)) {
      return data.map(num => num.toFixed(0)).join(', ');
    }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            ...
            dado que estos eventos ocurren con una tasa promedio conocida.
          </p>
        </div>
