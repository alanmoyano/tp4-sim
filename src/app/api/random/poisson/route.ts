  ...
  try {
    ...
    if (lambda <= 0) {
      return NextResponse.json(
        { error: 'El parámetro lambda debe ser mayor que 0' },
        { status: 400 }
      );
    }
    ...
    const numbers = Array.from({ length: count }, () => random.poisson(lambda)());
    ...
    console.error('Error generating Poisson random numbers:', error);
    return NextResponse.json(
